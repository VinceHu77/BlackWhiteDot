...
static func check_win(board: Array, x: int, y: int, win_count: int) -> int:
	var color = board[y][x]
	if color == WuziqiEatCfg.Player.NONE:
		return WuziqiEatCfg.Player.NONE
...
static func get_opponent(color: int) -> int:
	if color == WuziqiEatCfg.Player.BLACK:
		return WuziqiEatCfg.Player.WHITE
	elif color == WuziqiEatCfg.Player.WHITE:
		return WuziqiEatCfg.Player.BLACK
	return WuziqiEatCfg.Player.NONE
