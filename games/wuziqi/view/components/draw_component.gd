...
func draw_piece(x: int, y: int, color: int) -> void:
	var piece_scene := config.black_chess_scene if color == WuziqiCfg.Player.BLACK else config.white_chess_scene
	var visual = piece_scene.instantiate()
	var chess_piece := ChessPiece.new(Vector2i(x, y), color, config.cell_size)
